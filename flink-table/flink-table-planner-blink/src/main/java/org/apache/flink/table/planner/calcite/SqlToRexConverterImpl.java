...
/**
 * Standard implementation of {@link SqlToRexConverter}.
 */
public class SqlToRexConverterImpl implements SqlToRexConverter {
	...
	@Override
	public RelDataType getExpressionRowType(String statement) {
		String query = String.format(QUERY_FORMAT, statement);
		SqlNode parsed = planner.parse(query);
		SqlNode validated = planner.validate(parsed);
		return planner.validator().getValidatedNodeType(validated);
	}
	...
	private static CalciteCatalogReader createSingleTableCatalogReader(
		...
		CalciteSchema schema = CalciteSchemaBuilder.asRootSchema(new ExplicitTableSchema(tableMap));
